...
import { 
  ...
  ShieldAlert, Gavel, Scale, Coins 
} from 'lucide-react';

const Step5CreditHistory = ({ register, errors, watch }) => {
  // Observamos el estado de las quitas para mostrar campos adicionales
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-left">
          ...
          <div>
            ...
            <input 
              type="number"
              {...register("delinq_2yrs", { required: "Dato requerido", min: 0 })}
              className="mt-1 block w-full p-2 border border-gray-300 rounded-md focus:ring-[#1A4E5E] focus:border-[#1A4E5E]"
            />
          </div>

          {/* REGISTROS PÚBLICOS (Actualizado con icono) */}
          <div>
            <label className="block text-xs font-medium text-gray-700">Registros Públicos (Juicios/Bancarrotas)</label>
            <div className="relative">
              <Gavel className="absolute right-3 top-2.5 text-gray-400" size={14} />
              <input 
                type="number"
                {...register("pub_rec", { required: "Dato requerido", min: 0 })}
                className="mt-1 block w-full p-2 border border-gray-300 rounded-md focus:ring-[#1A4E5E] focus:border-[#1A4E5E]"
                placeholder="Ej. 0"
              />
            </div>
        ...
        <div className="bg-red-50 p-4 rounded-xl border border-red-100 space-y-4">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-[10px] text-red-600 uppercase font-black">Variable Crítica: has_settlements</p>
            </div>
            <input 
              type="checkbox"
              {...register("has_settlements")}
              className="w-6 h-6 text-red-600 border-gray-300 rounded-lg focus:ring-red-500"
            />
          </div>

          {/* Campos condicionales para Quitas */}
          {hasSettlements && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2 animate-in slide-in-from-top-2 duration-300">
              <div>
                <label className="block text-[10px] font-bold text-red-700 uppercase">Cantidad de Quitas Realizadas</label>
                <div className="relative mt-1">
                  <Scale className="absolute right-3 top-2.5 text-red-300" size={14} />
                  <input 
                    type="number"
                    {...register("settlement_count", { min: 0 })}
                    className="block w-full p-2 bg-white border border-red-200 rounded-md text-red-900 focus:ring-red-500 focus:border-red-500"
                    placeholder="Ej. 1"
                  />
                </div>
              </div>
              <div>
                <label className="block text-[10px] font-bold text-red-700 uppercase">Suma de Saldos Condonados (MXN)</label>
                <div className="relative mt-1">
                  <Coins className="absolute right-3 top-2.5 text-red-300" size={14} />
                  <input 
                    type="number"
                    {...register("settlement_amount", { min: 0 })}
                    className="block w-full p-2 bg-white border border-red-200 rounded-md text-red-900 focus:ring-red-500 focus:border-red-500"
