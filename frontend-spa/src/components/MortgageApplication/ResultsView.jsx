...
const ResultsView = ({ sentData, receivedData, onEdit }) => {
  ...
  const isEditable = ['draft', 'sent_awaiting_ia', 'assigning_broker', 'broker_assigned'].includes(receivedData?.status);
      ...
      <div className="print-container space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <SummaryCard icon={<Briefcase size={18} />} title="Situación Laboral" className="print-card">
            <DataRow label="Tipo de empleo" value={employmentLabels[sentData?.employment_type] || sentData?.employment_type} /> 
            <DataRow label="Antigüedad" value={`${sentData?.job_seniority_years} años`}  />
            <DataRow label="Empresa" value={sentData?.company_name}  />
            <DataRow label="Puesto" value={sentData?.job_title}  />
            <DataRow label="Nómina en este Banco" value={sentData?.payroll_at_bank ? "Sí" : "No"} />
          </SummaryCard>
          ...
          <SummaryCard icon={<DollarSign size={18} />} title="Ingresos y Capacidad Financiera" className="print-card">
            ...
            <DataRow label="Pago Mensual de Otras Deudas" value={formatCurrency(sentData?.installment)}  />
            <DataRow label="Fuente de Verificación" value={verificationLabels[sentData?.verification_status] || sentData?.verification_status} 
  />
          </SummaryCard>
          ...
          <SummaryCard icon={<History size={18} />} title="Información del Buró de Crédito" className="print-card">
            <DataRow label="Saldo Total Revolvente" value={sentData?.revol_bal ? `$${sentData.revol_bal}` : 0} />
            <DataRow label="Límite Total de Crédito" value={sentData?.total_rev_hi_lim ? `$${sentData.total_rev_hi_lim}` : 0} />
            <DataRow label="% de Utilización" value={sentData?.revol_util !== undefined ? `${sentData.revol_util}%` : '0%'} />
            ...
            <DataRow label="Saldo Total Actual" value={sentData?.tot_cur_bal ? `$${sentData.tot_cur_bal}` : 0} />
            <DataRow label="Monto en Cobranza (Collection)" value={sentData?.tot_coll_amt ? `$${sentData.tot_coll_amt}` : 0} />
            <DataRow label="Cobros últimos 12 meses (No médicos)" value={sentData?.collections_12_mths_ex_med} />
            <DataRow label="Quitas o Recuperaciones" value={sentData?.has_settlements ? "Sí" : "No"} />
          </SummaryCard>
        </div>
      </div>
...
const DataRow = ({ label, value }) => (
  <div className="flex justify-between items-center text-sm">
    ...
    <span className="text-slate-700 font-bold text-right">
      {/* Comprobamos si el valor es null o undefined estrictamente */}
